#!/home/gschroed/programs/EMAN2/extlib/bin/python2
# -*- coding: utf-8 -*-
	...
	def calcweight(self, point1, point2):
		...
		if self.cutoff(d):
			...
			for i in range(3,midp-2):
				...
				#print "np=", np
				#print int(round(np[0]/self.apix_x+SX/2))
				#np=[(x[0]+x[1]) for x in zip(p,a)]
				
				if self.mrcfile:
					#print "before get_value"
			...
			if mpt>self.mapthresh:
				# MODIFIED 04/05/16 GUNNAR
				dst=0 #mpt*10
				#dst=self.mapthresh/(mpt+.0001)#mpt*10
			else:
				# MODIFIED 04/05/16 GUNNAR
				dst=10000
				#dst=(self.mapthresh/(mpt+.0001))*1
			...
			#if (dst<0.0001): dst=0.0001
			#print "DST = ", dst
			#w= w * (1+dst*self.mrcweight)
			if (w<0) :
				w=0
		
		#print "WITH DENSITY = ", w
		w=int(w+0)
